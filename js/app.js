...
tbApp.controller('taskboardController', function ($scope, $filter, $http) {
    ...
    var updateConfig = function () {
        try {
            ...
            alert("Sorry, an error occured while updating the config: " + error)
        }
    }
    ...
    var setUrls = function () {
        ...
        $scope.VERSION_URL = 'http://janware.nl/gitlab/version.txt';
        $scope.DOWNLOAD_URL = 'http://janware.nl/gitlab/janban.zip';
        $scope.WHATSNEW_URL = 'http://janware.nl/gitlab/whatsnew.html';
        $scope.PING_URL = 'https://janvanveldhuizen.outsystemscloud.com//JanbanApi/rest/janbanping/hello?email={{email}}#PINGBACK#name={{name}}#PINGBACK#version={{version}}';
